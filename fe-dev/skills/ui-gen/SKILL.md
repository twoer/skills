...
3. **资源下载与 SVG 映射**（必须在编写 .vue 之前完成）：
   ...
   **SVG→代码映射表**（强制）：导出完成后，输出映射表并在编写代码时逐项对照引用：
   ```
   SVG 资源映射:
     frame-4-5048.svg (36x36) → StatCard[GMV] 图标
     frame-2-06725.svg (36x36) → StatCard[佣金] 图标
     frame-2-06762.svg (36x36) → StatCard[订单] 图标
     frame-2-06750.svg (36x36) → StatCard[退款] 图标
     ...
   ```
   **每个导出的 SVG 必须在生成代码中被引用**，不得用 EP 图标替代。只有 EP 组件内置的功能图标（如 ElSelect 的箭头、ElDatePicker 的日历）不需要替代。
4. **编写 .vue 文件**：
