...
从 ui-add 保存的中间文件读取 DSL 数据，**同时**提取 CSS 样式信息和资源信息。

**6a. 读取 dsl_raw.json**

路径：`{UI_DIR}/specs/{pageId}-dsl-raw.json`

- 存在 → 读取 JSON 内容，继续步骤 6b
- 不存在 → 跳过资源提取，仅依赖 spec 中已有的信息生成代码（降级模式）：

```
⚠️ 未找到 {UI_DIR}/specs/{pageId}-dsl-raw.json，跳过资源提取。
请先运行 /fe-dev:ui-add 重新生成设计规格。
```

**6b. 遍历 DSL 节点树，同时提取 CSS 样式和资源信息：**

**6b.1 CSS 样式提取**（按容器层级提取每个节点的关键 CSS 属性）：
...
10. 如果 dsl_raw.json 不存在或读取失败，跳过资源提取，仅依赖 spec 中已有的信息生成代码（降级模式，输出提示）
...
在下载资源完成后、生成代码前，评估页面复杂度，决定是否需要进行组件架构规划。
...
| # | 条件 | 检查方式 |
|---|------|---------|
| 1 | 资源清单 > 5 个资源 | spec "资源清单"行数 |
| 2 | 组件清单 > 10 行 | spec "组件清单"行数 |
| 3 | 语义组件树深度 > 4 层 | spec "布局结构"中的嵌套层级 |
| 4 | 包含图片+文字叠加区域 | 语义组件树中有 `(覆盖层)` 标注的节点 |
| 5 | 包含重复的卡片/列表单元 | 语义组件树中有多个同名同级节点（如多个 StatCard） |

**命中 ≥ 2 → 执行步骤 7.5b；否则跳过，直接进入步骤 8。**

**7.5b. 架构规划**

对 spec 进行深度分析，输出以下内容（不写入文件，仅作为步骤 8 的输入）：
...
|------|------|---------|
| 简单页面 | 命中 < 2 条 | 单文件，不拆分 |
| 中等页面 | 命中 2-3 条 | 按功能区域拆分为 2-3 个子组件 |
| 复杂页面 | 命中 ≥ 4 条 | 拆分为页面容器 + 多个子组件，需规划 props/emit |

**2) 组件拆分方案**

基于 spec 的语义组件树，识别可拆分的逻辑单元：

- **页面容器**（index.vue）— 组合子组件、管理页面级状态（loading、筛选条件）
- **功能子组件** — 按语义区域拆分，每个 PascalCase 节点树为一个候选组件
- **展示子组件** — 重复出现的卡片/列表项（如 StatCard、CampaignCard），提取为可复用组件
...
组件拆分方案:
  index.vue (页面容器)
    ├── FilterSection.vue (筛选区) — props: filterForm, emits: query, reset
    ├── StatCards.vue (统计卡片区) — props: summaryData, loading
    │   └── StatCard.vue (统计卡片) ← 可复用 — props: label, value, icon, color
    └── DataTable.vue (数据表格区) — props: tableData, loading
```
...
| 状态 | 归属 | 理由 |
|------|------|------|
| filterForm | index.vue | 跨组件共享，由页面容器管理 |
| loading | index.vue | 全局 loading 态 |
| tableData | index.vue → DataTable | 数据由页面获取，通过 props 下发 |
| columnDialogVisible | DataTable.vue | 弹窗仅表格区使用 |

**4) 复用识别**

从语义组件树中找出重复出现的结构模式：
- 多个同级同名节点 → 提取为可复用组件（v-for 渲染）
- 多处出现的图标+文字组合 → 提取为通用展示组件
...
🏗️ 组件架构规划:
  页面类型: 复杂页面
  拆分方案: 1 个页面容器 + 4 个子组件
  可复用组件: StatCard, CampaignCard

  组件结构:
    index.vue
      ├── FilterSection.vue
      ├── StatCards.vue → StatCard.vue (x4)
      └── DataTable.vue

  状态管理: 页面容器集中管理 filterForm/loading，子组件通过 props/emit 通信
```
...
**组件拆分策略**：
- 按步骤 7.5 的架构规划执行拆分（如已执行）
- 未执行步骤 7.5 时（简单页面）：单文件生成，不拆分
- 子组件文件放置在同目录的 `components/` 子目录下
- 子组件命名遵循 PascalCase，与语义组件树一致
- **样式归属**：Scoped SCSS tokens 写在使用该 token 的子组件中（如 `--stat-card-radius` 写在 StatCard.vue），页面容器只写页面级 token
- **多文件写入顺序**：先写子组件（叶子节点优先），再写页面容器（index.vue），确保 import 路径正确
