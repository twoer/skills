...
| ElInput | `size`, `clearable`, `show-password`, `type` | 按通用尺寸映射；高度 >60px → `type="textarea"` |
| ElSelect | `size`, `clearable`, `multiple`, `filterable` | 按通用尺寸映射 |
| ElTable | `border`, `stripe`, `size` | 有边框线 → `border`；交替行背景色不同 → `stripe` |
| ElDatePicker | `type`, `size`, `clearable` | 按通用尺寸映射 |
